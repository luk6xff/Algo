...
/*@brief Test method for my binary HEAp implementation*/
...
static float computeMedian(int val, MinHeap& minHeap, MaxHeap& maxHeap)
{
    float median = 0;   
    float minRoot = minHeap.getRootVal();
    int minSize = minHeap.getHeapSize();
    float maxRoot = maxHeap.getRootVal();
    int maxSize = maxHeap.getHeapSize();
#undef SHOW_STATE 
#ifdef SHOW_STATE 
    cout<<"min: "<<minRoot<<" maxRoot: "<<maxRoot<<endl;
    cout<<"MIN_HEAP: ";
    for(int i = 0; i< minHeap.getHeapSize(); ++i)
    {
        cout<< minHeap.getPtr()[i]<<" ";   
    ...
    if(minSize < maxSize)
    {
        ...
        median = ((minHeap.getRootVal() + maxHeap.getRootVal()) /2);
    }
    ...
    else //minSize == maxSize
    {
        ...
        {
            minHeap.insert(val);
            median = minHeap.getRootVal(); 
        }
        else
        {
            maxHeap.insert(val);
            median = maxHeap.getRootVal(); 
        }                             
    }
    return median;    
}
...
int main()
{
    ...
    {
        cin>>val;
        cout<<computeMedian(val, minHeap, maxHeap)<<endl;
    }
    return 0;
}
